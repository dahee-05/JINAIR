...
import {
  ...
  getArrive,
  resetSearchState,
} from "../../service/adminSearchApi.js";
...
export default function AdminFlightAdd() {
  ...
  useEffect(() => {
    axios
      .post("http://localhost:9000/admin/Fnum")
      .then((res) => {
        setFnum(res.data);
  ...
  useEffect(() => {
    ...
    adDeparture !== "" &&
      depart.current.style.setProperty(
        "border-bottom",
        "1px solid var(--color-153)",
      );
    adArrive !== "" && arr.current.style.setProperty("border", "none");
    adArrive !== "" &&
      arr.current.style.setProperty(
        "border-bottom",
        "1px solid var(--color-153)",
      );
    adStartDate !== "" && date.current.style.setProperty("border", "none");
    adStartDate !== "" &&
      date.current.style.setProperty(
        "border-bottom",
        "1px solid var(--color-153)",
      );
  ...
  useEffect(() => {
    setFormData((prev) => ({
      ...prev,
      Departure_location: adDeparture,
  ...
  const handleSubmit = (e) => {
    ...
    if (result) {
      ...
      axios
        .post("http://localhost:9000/admin/flightRegister", formData)
        .then((res) => {
    ...
    <div className="admin-airlist-content">
      <form onSubmit={handleSubmit}>
        <div className="admin-airlist-top">
          <div className="admin-airlist-title">항공권 등록</div>
        </div>
        <div className="admin-add-air">
          {adModalOpen && (
            <AdminSearchCountryModal mom={mom} departure={adDeparture} />
          )}
          {adCalendar3 && (
            <AdminOnewaySearchCalendar
              type="start"
              startCalendar={startCalendar}
            />
          )}
          {adCalendar4 && (
            <AdminOnewaySearchCalendar type="end" endCalendar={endCalendar} />
          )}
          ...
          <div className="main-top-search-bottom-main-middle2 add-contry contry-check">
            ...
            >
              <h5>출발지 선택</h5>
              <div>
                <h3 className="active-arrive-country">
                  {adDeparture === "" ? "출발" : adDeparture}
                </h3>
                <input
                  type="hidden"
                  name="Departure_location"
                  ref={refs.adDepartureRef}
                  value={adDeparture}
                />
            ...
            >
              ...
              <div>
                <h3 className="active-arrive-country">
                  {adArrive === "" ? "도착" : adArrive}
                </h3>
                <input
                  type="hidden"
                  name="Arrive_location"
                  ref={refs.adArriveRef}
                  value={adArrive}
                />
                <IoIosArrowDown className="main-top-search-bottom-main-middle-icon" />
              </div>
          ...
          {msgResult.adStartDate ? (
            <div className="validate-text error validate-text">
              {msgResult.adStartDate}
            </div>
          ) : msgResult.adStartTime ? (
            <div className="validate-text error">{msgResult.adStartTime}</div>
          ) : msgResult.adEndDate ? (
            <div className="validate-text error">{msgResult.adEndDate}</div>
          ) : msgResult.adEndTime ? (
            <div className="validate-text error">{msgResult.adEndTime}</div>
          ) : (
            ""
          )}
          ...
          <div className="add-flight-info">
            <div className="add-info">
              ...
              <div className="add-input-wrapper">
                ...
                <span className="admin-check" style={{ color: "red" }}>
                  *
                </span>
                <select
                  name="pnum"
            ...
            <div className="add-info2">
              ...
              {msgResult.basicPrice ? (
                <div className="validate-text error">
                  {msgResult.basicPrice}
                </div>
              ) : (
                <div className="validate-text"></div>
              ...
              <div className="add-input-wrapper">
                ...
                <span className="admin-check2" style={{ color: "red" }}>
                  *
                </span>
                <input
                  type="text"
