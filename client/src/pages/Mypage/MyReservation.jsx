...
export default function MyReservation() {
  ...
  useEffect(() => {
    if (isLoggedIn) {
      ...
      axios
        .post("http://localhost:9000/mypage/getMyRes", { id })
        .then((res) => {
  ...
  const filteredData = resData.filter((itemGroup) => {
    const departureDate = dayjs(itemGroup[0].departure_date)
      .add(9, "hour")
      .startOf("day");
        ...
        <section className="mp-common-content">
          ...
          <div className="myRes-top">
            <ul className="res-filter-btn">
              ...
              <li>
                <button
                  aria-selected={filterBtn === "지난예약"}
                  onClick={() => setFilterBtn("지난예약")}
                >
                  지난예약
                </button>
              </li>
            ...
            {filteredData.length > 0 ? (
              filteredData.map((itemGroup, i) => (
                <section className="myRes-tbody" key={i}>
                  ...
                  <div>
                    {dayjs(itemGroup[0].res_date).format("YYYY.MM.DD (ddd)")}
                  </div>
                  <div>{itemGroup.length === 2 ? "왕복" : "편도"}</div>
                  <div>
                    {itemGroup[0].passenger_name
                      ? itemGroup[0].passenger_name.length
                      : 0}
                    명
                  </div>
