    ...
    axios
      ...
      .then(({ data }) => {
        ...
        const filtered = data.filter((group) =>
          group.some((item) =>
            dayjs(item.departure_date).isSameOrAfter(today, "day"),
          ),
        );
            ...
            {resData ? (
              resData.map((itemGroup, i) => (
                <section className="myRes-tbody" key={i}>
                  ...
                  <div>
                    {dayjs(itemGroup[0].res_date).format(
                  ...
                  {/* 여정 출발일 기준 디데이 계산 */}
                  <div className="gap10">
                    {dayjs(itemGroup[0].departure_date).isSame(
                      dayjs(),
                      "day",
                    ) ? (
                      <>
                        ...
                        <button
                          className="w-btn"
                          onClick={() => setSendTarget(itemGroup[0])}
                        >
                          탑승권 발행
                        </button>
                      </>
