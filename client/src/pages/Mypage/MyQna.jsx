...
export default function MyQna() {
  ...
  useEffect(() => {
    if (isLoggedIn) {
      ...
      axios
        .post("http://localhost:9000/mypage/getMyQna", { id })
        .then((res) => {
            ...
            {qnaData && qnaData.length > 0 ? (
              qnaData.map((item, i) => (
                <section className="myQna-tbody" key={item.qid || i}>
                  <div
                    className="myQna-title-wrap cursor-pointer"
                    onClick={() => toggleContent(i)}
                  >
                    <div>{item.category}</div>
                    <div>{item.TITLE}</div>
                    <div>{item.id}</div>
                    <div>{dayjs(item.REG_DATE).format("YYYY.MM.DD HH:mm")}</div>
                    <div>{item.comment || "확인중"}</div>
                  </div>
                  {isOpen.includes(i) && (
