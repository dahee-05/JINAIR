...
/***************************
 * 2. 비행 리스트 조회 및 항공권 검색
 ***************************/
export const getFlightList = async ({ type, keyword, limit, offset }) => {
  let values = [];
  let sql = `
  ...
  if (type === "" && keyword !== "") {
    sql += ` WHERE ( departure_location LIKE ?
                  or arrive_location LIKE ?
                  or fnum LIKE ?)
...
/***************************
 * 2-1. 비행 리스트 TotalCnt 조회
 ***************************/
export const getFlightListTotalCnt = async ({ keyword, type }) => {
  let values = [];
  let sql = `select count(*) as cnt from flight`;
...
export const setFlightRegister = async (formData) => {
  ...
  if (result.affectedRows === 1) {
    const result = await db.execute(seatSql, [
      formData.fnum,
      JSON.stringify(seatList),
...
export const getNoticeList = async ({ offset, limit }) => {
  const sql = ` 
      select  ROW_NUMBER() OVER (ORDER BY reg_date desc) AS no
        , num , title , date_format(reg_date, '%Y-%m-%d %H-%I:%S') AS  reg_date 
      from (
        select num, title, reg_Date
          from notice 
         where type = 'n'
...
/***************************
 * 7-1. 공지사항 리스트 TotalCnt 조회
 ***************************/
export const getNoticeTotalCnt = async () => {
...
export const updateNoticeInfo = async (dataToSend) => {
  ...
  const values = [
    dataToSend.title,
    dataToSend.content,
    Number(String(dataToSend.num).replace(/^:/, "")),
  ];
