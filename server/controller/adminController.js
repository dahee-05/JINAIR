...
export const getFlightList = async (req, res) => {
  ...
  let result = await repository.getFlightList({ type, keyword, limit, offset });
  if (result.length > 0) {
    const cntResult = await repository.getFlightListTotalCnt({ type, keyword });
    result = { result, cnt: cntResult.cnt };
  }
  res.json(result);
};
...
export const setFlightRegister = async (req, res) => {
  ...
  const { name: departure_location, code: d_acode } =
    nameAndCode(Departure_location);
...
export const getNoticeList = async (req, res) => {
  ...
  const page = Number(req.query.page);
  const offset = (page - 1) * limit;
  let result = await repository.getNoticeList({ offset, limit });

  if (result.length > 0) {
    const cntResult = await repository.getNoticeTotalCnt();
    result = { result, cnt: cntResult.cnt };
  }
  res.json(result);
};
